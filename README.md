...
- **ダッシュボード**: 英雄カード＋3ステップ導線のみのシンプルなOverview。対象月、最新件数、不足状況を即座に把握でき、Builder / Monitor / Quick Access Hub へのCTAを集約。
- **Schedule Builder（/schedule-editor）**: 上部カードでシフト作成を行い、下部のグリッドで調整／保存まで完結。生成・初期化・CSVダウンロード・不足表示・自動グリッド再読込を1画面に統合。
- **運用モニター（/monitor）**: ダッシュボードから独立した統計ビュー。FREE/OFF日次表・勤務日数メーター・データ統計をモニター専用に整理。
